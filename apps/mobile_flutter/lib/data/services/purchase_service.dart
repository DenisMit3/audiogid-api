...
class PurchaseService extends _$PurchaseService {
  ...
  Future<void> _initIap() async {
    ...
    if (!available) {
      state = PurchaseState(
          status: PurchaseStatusState.error, error: "Store not available");
      return;
    }
    ...
    _subscription = _iap.purchaseStream.listen(
      ...
      onError: (error) {
        state = PurchaseState(
            status: PurchaseStatusState.error, error: error.toString());
      },
    );
  }
  ...
  Future<void> _onPurchaseUpdate(
    ...
    final restored = purchaseDetailsList
        .where((p) => p.status == PurchaseStatus.restored)
        .toList();
    final others = purchaseDetailsList
        .where((p) => p.status != PurchaseStatus.restored)
        .toList();
  ...
  Future<bool> _verifyPurchase(PurchaseDetails purchaseDetails) async {
    ...
    try {
      ...
      if (Platform.isIOS) {
        final response =
            await billingApi.verifyAppleReceipt(api.VerifyAppleReceiptRequest(
        ...
        final response = await billingApi
            .verifyGooglePurchase(api.VerifyGooglePurchaseRequest(
      ...
      state =
          PurchaseState(status: PurchaseStatusState.error, error: e.toString());
      return false;
    }
  }
  ...
  Future<void> buy(ProductDetails product) async {
    ...
    try {
      ...
      state = PurchaseState(
          status: PurchaseStatusState.error, error: "Buy failed: $e");
    }
  }
  ...
  Future<void> buyBatch(List<String> poiIds, List<String> tourIds) async {
    ...
    try {
      ...
      final res = await billingApi.batchPurchase(api.BatchPurchaseRequest(
        poiIds: poiIds,
        tourIds: tourIds,
        deviceAnonId: deviceId,
      ));
      ...
      if (productDetails.isEmpty) {
        state = PurchaseState(
            status: PurchaseStatusState.error,
            error: "Products not found in store");
        return;
      }
      ...
      state = PurchaseState(
          status: PurchaseStatusState.error, error: "Batch buy failed: $e");
    }
  }
  ...
  Future<void> restorePurchases() async {
    ...
    try {
      ...
      if (Platform.isIOS) {
        ...
        state = PurchaseState(
            status: PurchaseStatusState.error, error: "Platform not supported");
      }
    } catch (e) {
      state = PurchaseState(
          status: PurchaseStatusState.error, error: "Restore failed: $e");
    }
  }

  Future<void> _startServerRestoreAndroid(
      List<GooglePlayPurchaseDetails> purchases) async {
    final billingApi = ref.read(billingApiProvider);
    final prefs = await SharedPreferences.getInstance();
    final deviceId = prefs.getString('device_anon_id') ?? '';
    final packageInfo = await PackageInfo.fromPlatform();
    ...
    final googleItems = purchases
        .map((p) => api.GooglePurchaseItem(
              packageName: packageInfo.packageName,
    ...
    try {
      final result =
          await billingApi.restorePurchases(api.RestorePurchasesRequest(
      ...
      state = PurchaseState(
          status: PurchaseStatusState.error,
          error: "Server restore failed, check internet");
    }
  }
  ...
  Future<void> _pollRestoreJob(String jobId) async {
    if (jobId.isEmpty) {
      state = PurchaseState(
          status: PurchaseStatusState.error, error: "No job ID returned");
      return;
    }
    ...
    while (attempts < 30) {
      ...
      try {
        ...
        if (status?.status == api.RestoreJobReadStatusEnum.COMPLETED) {
          ...
          state = PurchaseState(
              status: PurchaseStatusState.error,
              error: status?.lastError ?? "Restore Job Failed");
          break;
        }
      } catch (e) {
        state = PurchaseState(
            status: PurchaseStatusState.error, error: "Polling failed");
        break;
      }
    }
    if (attempts >= 30) {
      state = PurchaseState(
          status: PurchaseStatusState.error, error: "Restore timed out");
    }
  }
}
