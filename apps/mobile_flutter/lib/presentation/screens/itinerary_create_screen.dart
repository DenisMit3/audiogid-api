...
class _ItineraryCreateScreenState extends ConsumerState<ItineraryCreateScreen> {
               ...
               Expanded(
                 child: FutureBuilder<List<Poi>>(
                   ...
                   builder: (context, snapshot) {
                      ...
                      if (snapshot.hasError) return ErrorStateWidget.generic(error: snapshot.error);
                      
                      final pois = snapshot.data ?? [];
                      // Sync order with ids
                      final orderedPois = ids.map((id) => pois.firstWhere((p) => p.id == id, orElse: () => pois.first)).toList();
                            ...
                            return ListTile(
                             ...
                             leading: Container(
                                ...
                                decoration: BoxDecoration(
                                  ...
                                  image: poi.media.isNotEmpty ? DecorationImage(image: NetworkImage(poi.media.first.url), fit: BoxFit.cover) : null,
  ...
  Future<void> _saveItinerary(List<String> poiIds) async {
    ...
    if (title.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Введите название маршрута')));
      return;
    }
    ...
    try {
      ...
      final id = result['id'];
      final shareUrl = "https://audiogid.app/itinerary/$id"; // TODO: Use dynamic links or config
        ...
        builder: (ctx) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              SelectableText(shareUrl, style: const TextStyle(fontWeight: FontWeight.bold)),
            ],
          ),
          actions: [
            TextButton(
