...
class ItineraryViewerScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ItineraryViewerScreen> createState() => _ItineraryViewerScreenState();
}
...
class _ItineraryViewerScreenState extends ConsumerState<ItineraryViewerScreen> {
  ...
  void initState() {
    super.initState();
    _itineraryFuture = ref.read(itineraryRepositoryProvider.future).then((repo) => repo.getItinerary(widget.itineraryId));
  }
      ...
      body: FutureBuilder<Map<String, dynamic>>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) {
            return ErrorStateWidget.generic(error: snapshot.error, onRetry: () {
               setState(() {
                 _itineraryFuture = ref.read(itineraryRepositoryProvider.future).then((repo) => repo.getItinerary(widget.itineraryId));
               });
            });
          ...
          if (items.isEmpty) {
            return const Center(child: Text('Этот маршрут пуст или недоступен.'));
          }
                     ...
                     return ListTile(
                       leading: Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            image: (poiData['cover_image'] != null) ? DecorationImage(
                              image: NetworkImage(poiData['cover_image']),
                              fit: BoxFit.cover,
                            ) : null,
                          ),
                          child: poiData['cover_image'] == null ? const Icon(Icons.place) : null,
                       ),
                       title: Text(poiData['title_ru'] ?? 'POI'),
                       subtitle: Text(poiData['category'] ?? 'Место'),
                       trailing: Text('${index + 1}', style: Theme.of(context).textTheme.bodyLarge),
                     );
                   },
                 ),
               ),
