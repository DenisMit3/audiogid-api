...
class MockStorageManagerNotifier extends Mock
    implements StorageManager {} // Notifier mock?
...
class FakeBuildOfflineBundleRequest extends Fake
    implements BuildOfflineBundleRequest {}
...
void main() {
  ...
  setUp(() {
    ...
    TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
        .setMockMethodCallHandler(channelDownloader,
    ...
    TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
      ...
      return {
        15: 1
      }; // 1 = granted, 15 = storage? Value checks depend on plugin internals
    });
    ...
    container = ProviderContainer(
      overrides: [
        ...
        storageManagerProvider
            .overrideWith(() => mockStorageManager), // This mocks the Notifier?
  ...
  group('DownloadService', () {
    test('startDownload should enqueue and poll job', () async {
      ...
      // Arrange
      when(() => mockOfflineApi.buildOfflineBundle(
              buildOfflineBundleRequest:
                  any(named: 'buildOfflineBundleRequest')))
    ...
    test('should retry on network error during polling', () async {
      ...
      verify(() => mockOfflineApi.getOfflineBundleStatus(jobId: 'job_retry'))
          .called(2);
    });
  });
}
