...
void main() {
  ...
  setUp(() {
    ...
    registerFallbackValue(
        VerifyAppleReceiptRequest((b) => b..idempotencyKey = 'test'));
    registerFallbackValue(
        VerifyGooglePurchaseRequest((b) => b..idempotencyKey = 'test'));
    ...
    container = ProviderContainer(
      overrides: [
        ...
        entitlementRepositoryProvider
            .overrideWithValue(mockEntitlementRepository),
      ],
    );
  });
  ...
  group('PurchaseService', () {
    ...
    test('should verify purchase on stream update', () async {
      ...
      when(() => mockBillingApi.verifyGooglePurchase(
              verifyGooglePurchaseRequest:
                  any(named: 'verifyGooglePurchaseRequest')))
      ...
      when(() => mockBillingApi.verifyAppleReceipt(
              verifyAppleReceiptRequest:
                  any(named: 'verifyAppleReceiptRequest')))
          .thenAnswer((_) async => Response<PurchaseVerifyResponse>(
                requestOptions: RequestOptions(path: ''),
                data: verifyResponse,
                statusCode: 200,
      ...
      when(() => mockEntitlementRepository.syncGrants())
          .thenAnswer((_) async => {});
    ...
    test('should execute parallel purchases in buyBatch', () async {
      ...
      when(() => mockBillingApi.batchPurchase(
          batchPurchaseReq:
              any(named: 'batchPurchaseReq'))).thenAnswer((_) async => Response(
          requestOptions: RequestOptions(path: ''),
      ...
      when(() => mockInAppPurchase.buyNonConsumable(
      ...
      // Assert
      verify(() => mockBillingApi.batchPurchase(
          batchPurchaseReq: any(named: 'batchPurchaseReq'))).called(1);
      ...
      verify(() => mockInAppPurchase.buyNonConsumable(
