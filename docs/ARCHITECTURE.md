...
### 2026-02-25
- **Исправлена авторизация в админ-панели**: удалены все обращения к `localStorage.getItem('admin_token')`, теперь используется `credentials: 'include'` для передачи HttpOnly cookie
- **Исправлен WebSocket**: добавлен `withCredentials: true` в `useJobsWebSocket.ts`
- **Исправлен endpoint городов**: исправлен подсчет POI и туров в `admin/cities.py` - использование правильных SQL запросов вместо relationship objects
- **Исправлены bulk-операции POI**: теперь отправляются реальные UUID вместо индексов строк таблицы
- **Исправлена сериализация POI**: добавлен `exclude={'geo'}` в `poi.json()` для избежания ошибок сериализации GeoAlchemy2
- **Исправлена загрузка медиа**: поле `source_page_url` сделано опциональным в `CreateMediaReq` и `PoiMedia`
- **Удалены дублирующие роуты**: удалены устаревшие POI роуты из `publish.py` (использовали `x-admin-token`), теперь все POI операции через `admin/poi.py` с JWT auth
- **Улучшены сообщения валидации публикации**: все сообщения переведены на русский язык с конкретными подсказками
- **Исправлено модальное окно публикации**: добавлена передача `can_publish` и `publish_issues` в компонент формы POI
...
*Документация обновлена: 2026-02-25*
