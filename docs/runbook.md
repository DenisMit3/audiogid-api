...
### 6. Admin Panel Authentication (2026-02-25)
Админ-панель использует JWT авторизацию через HttpOnly cookies:

1. **Логин**: `POST /v1/auth/login/email` возвращает токены
2. **Cookie**: Фронтенд сохраняет `access_token` в HttpOnly cookie `token`
3. **Proxy**: Все запросы к `/api/proxy/*` автоматически извлекают токен из cookie и передают в `Authorization: Bearer` заголовке
4. **Credentials**: Все fetch-запросы используют `credentials: 'include'`

**Важно**: НЕ использовать `localStorage` для хранения токенов - это устаревший подход.

### 7. Публикация контента
Для публикации POI/Tour необходимо выполнить требования валидации:

**POI:**
- Название (RU) - минимум 3 символа
- Описание (RU) - минимум 10 символов
- Координаты - выбрать точку на карте
...
- Все точки должны быть опубликованы
